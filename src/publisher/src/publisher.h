 ...
 */
char* publish_json(const char *URL, const char *message);

/**
 * @brief Creates a new index in Elasticsearch if it not yet exists.
 */
// void mf_create_metrics_index(const char* URL, const char* schemaLocation);

/**
 * @brief Head request to check the availability of the given resource.
 */
int mf_head(const char* URL);

/**
 * @brief Registers a new workflow at the database
 */
char* mf_register_workflow(
    const char* URL,
    const char* workflow,
